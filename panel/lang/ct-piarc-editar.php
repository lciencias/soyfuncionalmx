...
if(isset($_GET) && isset($_GET['id']) && (int) $_GET['id'] > 0 ){
	$comite = new Comite($db,$_SESSION,$_GET,Comunes::LISTAR,Comunes::EDIT);
	$registro    = $comite->obtenRegistros();
	$temas  = $comite->obtenTemas();
	$cadenaRedes = $comite->obtenRedes();
	$cadenaCargo = $comite->obtenCargos();
}

if(isset($_POST) && isset($_POST['nombre']) && isset($_POST['presidente'])){
	if(isset($_FILES) && isset($_FILES['fileImg']) && trim($_FILES['fileImg']['name']) != ''){
	...
	if((int) $idImagen >= 0){
		$comite = new Comite($db,$_SESSION,$_POST,$idImagen,Comunes::UPDATE);
	...
	header("Location: ".$_SESSION['pathWeb']."ct-piarc-lista.php");
}
								...
								<div class="panel-body">
									<div class="form-wrap">
										<form enctype="multipart/form-data" action="<?=$_SERVER['PHP_SELF']?>" method="post" name="formaSlider" id="validateFormComite" autocomplete="off" data-toogle="validator" role="form">
											<input type="hidden" name="idcomite" id="idcomite" value="<?=$registro['idcomite']?>">
											<input type="hidden" name="idimagen" id="idimagen" value="<?=$registro['idimagen']?>">
											<input type="hidden" name="tipo" id="tipo" value="2">
											<input type="hidden" name="redes" id="redes" value="<?=$cadenaRedes?>">
											<input type="hidden" name="cargos" id="cargos" value="<?=$cadenaCargo?>">
        									...
        									<div class="row">
                                            	<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" id="nombre" name="nombre" tabindex="1" class="form-control required letras" placeholder="Nombre" value="<?=$registro['nombre']?>">
													</div>
												</div>
                                                <div class="col-md-6">
													<div class="form-group">
														<label class="control-label mb-10">Presidente</label>
														<input type="text" id="presidente" name="presidente" tabindex="2" class="form-control required letras" placeholder="Presidente" value="<?=$registro['presidente']?>">
                                                	</div>
												</div>
                                                <div class="col-md-12">
                                                	<div class="form-group">
														<label class="control-label mb-10 text-left">Direcci&oacute;n</label>
													    <textarea class="form-control alfa" id="direccion" name="direccion" tabindex="3" rows="5"><?=$registro['direccion']?></textarea>
												    </div>
                                                </div>
                                                <div class="col-md-12">
                                                	<div class="form-group">
														<label class="control-label mb-10 text-left">Objetivo</label>
													    <textarea class="form-control alfa" id="objetivo" name="objetivo" tabindex="4" rows="5"><?=$registro['objetivo']?></textarea>
												    </div>
                                                </div>
                                                <div class="col-md-6">
													<div class="form-group">
														<label class="control-label mb-10">Tel&eacute;fono</label>
														<input type="text" id="telefono" name="telefono" tabindex="5" class="form-control numeros" placeholder="Tel&eacute;fono" value="<?=$registro['telefono']?>">
													</div>
												</div>
                                                <div class="col-md-6">
													<div class="form-group">
														<label class="control-label mb-10">Correo</label>
														<input type="text" id="correo" name="correo" tabindex="6" class="form-control correo" placeholder="Correo" value="<?=$registro['correo']?>">
													</div>
												</div>
                                                <div class="col-md-6">
													<div class="form-group">
														<label class="control-label mb-10">URL Sitio Web</label>
														<input type="text" id="url" name="url" tabindex="7" class="form-control url"  placeholder="(con http)" value="<?=$registro['url']?>">
													</div>
												</div>
                                                <div class="col-md-6">
													<div class="form-group">
														<label class="control-label mb-10">Tema</label>
														<select name="idtema" id="idtema" class="form-control" tabindex="8">
														<?php 
														foreach($temas as $key => $value){
															$tmpS = "";
															if($key == $registro['idtema']){
																$tmpS = " selected ";
															}
														?>
															<option value="<?=$key?>" <?=$tmpS?>><?=$value?></option>
														<?php	
														}
														?>
												...
												<div class="row">
													<div class="col-lg-12">
														<div class="img-upload-wrap">
															<?php if((int)$registro['idimagen'] > 0){
															?>
															<img class="img-responsive" src="<?=$registro['web']?>" alt="upload_img">
															<?php 
															}else{
															?>
                                            ...
                                            <div class="form-actions">
												<button type="submit" class="btn btn-success btn-icon left-icon mr-10 pull-left" id="crearComite" name="crearComite"> <i class="fa fa-check"></i> <span>Actualizar</span></button>
...
<div class="modal fade" id="editor-modal" tabindex="-1" role="dialog" aria-labelledby="editor-title">
	<div class="modal-dialog" role="document">
		<form class="modal-content form-horizontal" id="editor">
			...
			<div class="modal-footer">
				<button type="button" id="actualizaMesa" name="actualizaMesa" class="btn btn-primary">Guardar</button>
